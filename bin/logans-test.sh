...
case "$1" in
    ...
    start)
        nohup $JRE_HOME/bin/java -Xms1024m -Xmx1024m -Dfile.encoding=UTF-8 -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=$SERVICE_DIR/oom/ -jar $JAR_NAME --spring.profiles.active=test  --spring.cloud.config.profile=test  >/dev/null 2>&1 &
